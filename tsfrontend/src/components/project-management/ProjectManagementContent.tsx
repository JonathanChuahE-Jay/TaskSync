...
import { IconCalendarEvent, IconChartBar, IconClipboardList } from '@tabler/icons-react';
      ...
      <div key={item} className="p-4 border border-gray-200 rounded-lg hover:shadow-md transition-shadow">
        ...
        <div className="mt-3 flex items-center gap-4">
          <div className="flex -space-x-2">
            {[1, 2, 3].map((avatar) => (
              <div key={avatar} className="w-6 h-6 rounded-full bg-indigo-300 border border-white"></div>
            ))}
          </div>
  ...
  <div className="border border-gray-200 rounded-lg p-4">
    ...
    <div className="grid grid-cols-7 gap-1">
      ...
      {Array.from({ length: 30 }, (_, i) => i + 1).map((date) => (
        <div
          key={date}
          className={`h-16 border border-gray-100 rounded p-1 text-sm hover:bg-gray-50 ${date === 15 || date === 22 ? 'bg-indigo-50 border-indigo-200' : ''
            }`}
        >
          <div className="font-medium">{date}</div>
          {date === 15 && <div className="text-xs p-1 bg-indigo-100 rounded mt-1 text-indigo-800">Project Review</div>}
          {date === 22 && <div className="text-xs p-1 bg-blue-100 rounded mt-1 text-blue-800">Deadline</div>}
        </div>
      ))}
    </div>
  </div>
...
const TasksView = () => (
  <div className="border border-gray-200 rounded-lg p-4">
    ...
    <div className="space-y-3">
      {[
        { name: 'Design wireframes', status: 'Completed', assigned: 'Alex', priority: 'High' },
        { name: 'API integration', status: 'In Progress', assigned: 'Sam', priority: 'High' },
        { name: 'User testing', status: 'Pending', assigned: 'Jess', priority: 'Medium' },
        { name: 'Documentation', status: 'Not Started', assigned: 'Taylor', priority: 'Low' },
        { name: 'Deployment', status: 'Pending', assigned: 'Team', priority: 'Critical' },
      ].map((task, i) => (
        <div key={i} className="flex items-center justify-between p-3 border border-gray-100 rounded-md hover:bg-gray-50">
          <div className="flex items-center gap-3">
            <input type="checkbox" checked={task.status === 'Completed'} className="rounded text-indigo-600" />
...
const ProjectManagementContent = ({ activeTab }: {activeTab: projectManagementTabsIDType}) => {
